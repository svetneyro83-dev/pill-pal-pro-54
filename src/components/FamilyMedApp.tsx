    ...
    <div className="min-h-screen bg-[#F8FAFC] text-[#1E293B] antialiased pb-20 font-sans">
      <div className="max-w-6xl mx-auto px-4 pt-6 md:pt-10">
                      ...
                      <div key={med.id} className={`bg-white rounded-[2.5rem] border transition-all ${isLow ? 'border-rose-200 shadow-md shadow-rose-50' : 'border-slate-100 shadow-sm'}`}>
                        <div className="p-6 md:p-8">
                          ...
                          <div className="mt-4 flex flex-wrap gap-2 pt-4 border-t border-slate-100">
                            <button
                              onClick={() => addAllToGoogle(med)}
                              className="flex-1 min-w-[160px] flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl bg-white border border-slate-200 text-xs font-bold text-slate-700 hover:bg-slate-50 transition-all"
                            >
                              <ExternalLink className="w-3.5 h-3.5" /> Google Календарь
                            </button>
                            <button
                              onClick={() => downloadICS(med)}
                              className="flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl bg-slate-900 text-white text-xs font-bold hover:bg-slate-800 transition-all"
                            >
        ...
        {view === 'calendar' && (() => {
          ...
          return (
            <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm p-6 md:p-8 animate-in fade-in slide-in-from-bottom-5">
              <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setCalendarMonth(new Date(calendarMonth.getFullYear(), calendarMonth.getMonth() - 1, 1))}
                    className="w-10 h-10 rounded-xl bg-slate-50 hover:bg-slate-100 font-bold"
                  >‹</button>
                  <h2 className="text-xl font-bold capitalize text-slate-800 min-w-[180px] text-center">{monthName}</h2>
                  <button
                    onClick={() => setCalendarMonth(new Date(calendarMonth.getFullYear(), calendarMonth.getMonth() + 1, 1))}
                    className="w-10 h-10 rounded-xl bg-slate-50 hover:bg-slate-100 font-bold"
                  >›</button>
                ...
                <button
                  onClick={() => downloadICS()}
                  className="flex items-center gap-2 py-2.5 px-4 rounded-xl bg-indigo-600 text-white text-xs font-bold hover:bg-indigo-700 transition-all shadow"
                >
                  <Download className="w-3.5 h-3.5" /> Экспорт всего курса (.ics)
                </button>
              </div>
              <div className="grid grid-cols-7 gap-1 mb-2">
                {['Пн','Вт','Ср','Чт','Пт','Сб','Вс'].map(d => (
                  <div key={d} className="text-[10px] font-black uppercase text-slate-400 text-center py-2">{d}</div>
                ))}
              </div>
              <div className="grid grid-cols-7 gap-1">
                {grid.map((day, i) => {
                  if (!day) return <div key={i} className="aspect-square" />;
                  const dayMeds = medsOnDay(day);
                  const isToday = sameDay(day, today);
                  return (
                    <div
                      key={i}
                      className={`aspect-square rounded-xl border p-1.5 flex flex-col gap-1 overflow-hidden ${
                        isToday ? 'bg-indigo-50 border-indigo-300' : 'bg-white border-slate-100'
                      }`}
                    >
                      <div className={`text-[11px] font-bold ${isToday ? 'text-indigo-700' : 'text-slate-500'}`}>{day.getDate()}</div>
                      <div className="flex flex-wrap gap-0.5">
                        {dayMeds.slice(0, 4).map(med => {
                          const m = getMember(med.memberId);
                          const c = colors.find(c => c.name === m.color)!;
                          return <div key={med.id} title={`${med.name} (${m.name}) — ${med.timing}`} className={`w-2 h-2 rounded-full ${c.bg}`} />;
                        })}
                        {dayMeds.length > 4 && <span className="text-[8px] text-slate-400 font-bold">+{dayMeds.length - 4}</span>}
                      </div>
                    </div>
                  );
                })}
              ...
              {meds.length > 0 && (
                <div className="mt-6 pt-6 border-t border-slate-100">
                  <h4 className="text-[10px] font-black uppercase text-slate-400 mb-3 tracking-widest">Курсы</h4>
                  <div className="space-y-2">
                    {meds.map(med => {
                      const m = getMember(med.memberId);
                      const c = colors.find(c => c.name === m.color)!;
                      const end = new Date(new Date(med.startDate).getTime() + (med.duration - 1) * 86400000);
                      return (
                        <div key={med.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
                          <div className="flex items-center gap-3 min-w-0">
                            ...
                            <div className="min-w-0">
                              <div className="font-bold text-sm text-slate-800 truncate">{med.name} <span className="text-slate-400 font-normal">— {m.name}</span></div>
                              <div className="text-[10px] text-slate-500 font-medium">
                                {new Date(med.startDate).toLocaleDateString('ru-RU')} → {end.toLocaleDateString('ru-RU')} • {med.timing} • {med.frequency}×/день
                              </div>
                            </div>
                          </div>
                          <button
                            onClick={() => addAllToGoogle(med)}
                            className="shrink-0 flex items-center gap-1.5 py-2 px-3 rounded-lg bg-white border border-slate-200 text-[11px] font-bold text-slate-700 hover:bg-indigo-50 hover:border-indigo-200 hover:text-indigo-700 transition-all"
                          >
